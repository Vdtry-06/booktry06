...
import vdtry06.springboot.gateway.api_gateway.service.AuthService;
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        return authService.introspect(token).flatMap(introspectResponse -> {
            if (introspectResponse.getData().isValid())
                ...
                return unauthenticated(exchange.getResponse());
        }).onErrorResume(throwable -> unauthenticated(exchange.getResponse()));// loi mat ket noi nen chan lai
